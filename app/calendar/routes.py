...
from app.models.calendar_type import CalendarType
from app.helpers.ical_util import ICalUtil
...
def create_route(app, endpoint):
    ...
    def serve_ical():
        ...
        implementation_param = request.args.get('implementation', ImplementationType.ICALENDAR.value)  # Default to 'icalendar'  

        currencies = None
        impact_classes = None

        if currencies_param:
            currencies = [Currencies.from_text(currency) for currency in currencies_param.split(',')]
        if impact_classes_param:
            impact_classes = [ImpactClass.from_text(impact_class) for impact_class in impact_classes_param.split(',')]
